...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        """
        if len(self.q) > 0 and (tuple(state),tuple(action)) in self.q:
    ...
    def best_future_reward(self, state):
        ...
        for state_action, q_value in self.q.items():
            if tuple(state) in state_action:
                best_future_reward = max(best_future_reward, q_value)
    ...
    def best_action(self, state):
        ...
        max_q = 0
        for state_action,q_value in self.q.items():
            if tuple(state) in state_action and q_value > max_q:
                max_q = q_value
