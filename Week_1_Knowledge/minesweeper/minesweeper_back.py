# -*- coding: utf-8 -*-
"""
Created on Sun May 17 17:05:36 2020
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        #print(type(neighbour_cells))
        self.knowledge.append(Sentence(neighbour_cells, count))
        knwldge = copy.deepcopy(self.knowledge)
        for sentence in knwldge:
            for sentence2 in knwldge:
                new_cells = []
                if(len(sentence2.cells) != 0 and len(sentence.cells) != 0 and sentence.cells != sentence2.cells and MinesweeperAI.issubset(sentence, sentence2)):
                    #print('hey1')
                    #print(f'{sentence.cells} - {sentence.count}')
                    #print(f'{sentence2.cells} - {sentence2.count}')
                    for s_cell in sentence.cells:
                        if s_cell not in sentence2.cells:
                            #print('duh')
                            new_cells.append(s_cell)
                    if(len(new_cells) == 1 and    sentence.count - sentence2.count == 1):
                        
                        MinesweeperAI.mark_mine(self, new_cells[0])
                    elif(len(new_cells) == 1 and    sentence.count - sentence2.count == 0):
                        ...
                        print(f'duh')
                        self.knowledge.append(Sentence(new_cells, sentence.count - sentence2.count))
            ...
            if(sentence.count == 0):
                for cell in sentence.cells:
                    if cell not in self.safes:
                        ...
                        MinesweeperAI.mark_safe(self, cell)
            if(sentence.count == range(len(sentence.cells))):
                for cell in sentence.cells:
    ...
    def issubset(superset,subset):
        return all(item in superset.cells for item in subset.cells)
    
    def madewrongmove(self, cell):
        print(f'You chose {cell}\n the safe cells are')
        for eachsafecell in self.safes:
            print(eachsafecell)
